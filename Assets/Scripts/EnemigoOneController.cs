...
public class EnemigoOneController : MonoBehaviour
{
    private float vidaEnemyOne; 
    ...
    void Update()
    {
        ...
        if (distanciaJugador < rangoVision )
        {
            if (sr.flipX == false)
            {
                ...
                if (tiempo >= 1)
                {
                    var position = new Vector2(transform.position.x -2, transform.position.y+1);
                ...
                if (tiempo2 >= 3)
                {
                    var position = new Vector2(transform.position.x -2, transform.position.y-2);
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        {
            vidaEnemyOne = vidaEnemyOne + 1;
            Debug.Log("vida enemigo: " + vidaEnemyOne);
